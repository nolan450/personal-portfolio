...
/* $(".contact_btn").on('click', function () {
    ...
    if (proceed === "true") {
        ...
        if ($("#result").length) {
            // alert("yes");
            output = '<div class="alert-danger" style="padding:10px 15px; margin-bottom:30px;">Veuillez remplir les champs manquants.</div>';
            $("#result").hide().html(output).slideDown();
            $(".contact_btn i").addClass('d-none');
        } else {
            Swal.fire({
                ...
                type: 'error',
                title: 'Oops...',
                html: '<div class="text-danger">Veuillez remplir les champs manquants.</div>'
            })
            $(".contact_btn i").addClass('d-none');
        }
